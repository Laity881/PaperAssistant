...
from core.llm_client import answer_macro_question, generate_macro_reading
...
with right:
    panel_title("DeepSeek 工作区", "宏观摘要、架构、创新点")
    ...
    if run_macro:
        try:
            with st.status("DeepSeek 正在分析论文...", expanded=True) as gen_status:
                gen_status.write("提取论文全文...")
                gen_status.write("生成宏观摘要...")
                answer = generate_macro_reading(paper_text)
...
if prompt:
    ...
    try:
        with st.spinner("DeepSeek 正在结合上下文回答..."):
            answer = answer_macro_question(
                prompt,
                paper_text,
                st.session_state[history_key],
            )
